...
function start(render) {
  ...
  if (isBrowser) {
    ...
    channel.on(_coreEvents2.default.SET_CURRENT_STORY, function (data) {
  ...
  var renderUI = function renderUI(forceRender) {
    if (isBrowser) {
      ...
      try {
        ...
        _addons2.default.getChannel().emit(_coreEvents2.default.STORY_RENDERED);
      } catch (ex) {
        showException(ex);
      }
    }
  ...
  if (isBrowser) {
    channel.on(_coreEvents2.default.FORCE_RE_RENDER, forceReRender);
  }
  renderUI();
  reduxStore.subscribe(renderUI);

  return { context: context, clientApi: clientApi, configApi: configApi, forceReRender: forceReRender };
}